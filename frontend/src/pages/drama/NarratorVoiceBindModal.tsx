...
function readNarrationVoiceBinding(project: DramaProject): VoiceBinding | null {
  ...
  const label = typeof data.label === 'string' ? data.label : "Narration Voice"
...
export function NarratorVoiceBindModal({ project, open, onClose, onUpdated, onError }: Props) {
  ...
  const handleConfirm = useCallback(async () => {
    if (!selectedVoice?.url || busy) {
      onError("Please select a narrator voice with a synthesized preview")
      return
    }
    setBusy(true)
    try {
      const binding: VoiceBinding = {
        ...
        label: selectedVoice.name || "Narration Voice",
  ...
  return (
    ...
    >
      <p className="drama-muted">
        {"The global narrator voice will be injected as Seedance’s <strong>reference_audio</strong> and used to keep narration consistent across all shots."}<strong>reference_audio</strong> {"Inject, for narration voiceover consistency across all shots."}</p>
      ...
      <div className="drama-voice-list" style={{ marginTop: 10 }}>
        {voiceAssets.length === 0 ? (
          <p className="drama-muted">{"No voice assets yet. Please synthesize a narrator voice in the \"Voice\" category first."}</p>
        ) : (
          voiceAssets.map((voice) => {
            ...
            return (
              <label key={voice.id} className="drama-voice-option" style={{ cursor: 'pointer' }}>
                ...
                />
                <span>
                  {voice.name || `Voice #${voice.id}`} <small>{hasAudio ? "Synthesized" : "Not synthesized"}</small>
                </span>
              </label>
            )
          })
